...
double
AltitudeMap::getaltitude(int x, int y) {
	if (x < xsize && y < ysize) {
		return ALT(x, y);
	}
	return 0;
...
double
AltitudeMap::getdiff(int x, int y) {
	if (dmap == NULL) {
		makediff();
	}
	if (x < xsize && y < ysize) {
...
void
AltitudeMap::setHumanPos(double x, double y) {
	human_pos_x = x;
	human_pos_y = y;
}

void
AltitudeMap::setAlienPos(double x, double y) {
	alien_pos_x = x;
	alien_pos_y = y;
}

double
AltitudeMap::getHumanPosX(void) {
	return human_pos_x;
}

double
AltitudeMap::getHumanPosY(void) {
	return human_pos_y;
}

double
AltitudeMap::getAlienPosX(void) {
	return alien_pos_x;
}

double
AltitudeMap::getAlienPosY(void) {
	return alien_pos_y;
}

void
AltitudeMap::makediff(void) {
	double itz = 10;
...
void
AltitudeMap::setwater(int x, int y, char level) {
	if (x < xsize && y < ysize) {
		IWATER(x, y) = level;
	}
}
...
char
AltitudeMap::getwater(int x, int y) {
	if (x < xsize && y < ysize) {
		return IWATER(x, y);
	}
	return 0;
...
int
AltitudeMap::gettex(int x, int y) {
	if (tmap == NULL) {
		maketextures();
	}
	if (x < xsize && y < ysize) {
...
void
AltitudeMap::subdivision(double coeff, double lt, double rt, double lb, double rb) {
	ALT(xsize - 1, 0) = rt;
	ALT(0, ysize - 1) = lb;
...
void
AltitudeMap::subdiv_private(double coeff, int x1, int y1, int x2, int y2) {
	if (abs(double(x1 - x2)) <= 1 || abs(double(y1 - y2)) <= 1) {
		return;
	}
...
void
AltitudeMap::randomize(double r) {
	for (int x = 0; x < xsize; ++x) {
		for (int y = 0; y < ysize; ++y) {
...
void
AltitudeMap::erosion(int r, int iter) {
	double *copy;
