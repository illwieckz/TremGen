...
float
getNormalDiff(AltitudeMap *hmap, double x, double y, double tx, double ty, double itz) {
	// fprintf(stderr,"--%f %f\n",tx,ty);
...
double
getRealAlt(AltitudeMap *hmap, double x, double y, double tx, double ty, double itz) {
	// fprintf(stderr, "--%f %f\n", tx, ty);
	int xx = (int)floor(x / tx);
	int yy = (int)floor(y / ty);
...
void
plantForest(double cx, double cy, int numTree, AltitudeMap *hmap, Entities_group *egp) {
	stringstream buf;
	Entity *etmp;
	double k;
...
void
dropBox(AltitudeMap *hmap, Entities_group *egp) {
	double ncx, ncy;
	int lx, ly;
	Entity *etmp;
	double phx, phy, pax, pay;
...
double
mesureAlien(double x, double y, double max, AltitudeMap *hmap) {
...
double
mesureHuman(double x, double y, double dist, double max, AltitudeMap *hmap) {
	double al = real(x * TSIZE, y * TSIZE);
	...
	// fprintf(stderr, "--%f %f\n", moy, dist);
	return (((al / 5.0) / (max * HINC)) + dist / moy) / 2.0; // we j'approx faudrait la diag et pas la moyenne
}

void
makeBasicEntities(AltitudeMap *hmap, Entities_group *egp) {
