...
std::ostream&
operator<<(std::ostream& out, args c)              {
	out << "\"" << c.name << "\" \"" << c.value << "\"";
	return out;
}
...
void
Entity::attrAdd(string name, string value) {
...
void
Entity::attrAdd(string name, double value) {
	if (name.size() == 1) {
		switch (name[0]) {
		case 'x':
			x = value;
...
void
Entity::attrMod(string name, string value) {
	attrAdd(name, value);
}

void
Entity::attrMod(string name, double value) {
	attrAdd(name, value);
}

args *
Entity::attrFind(string name, string value) {
	list<args>::iterator iter;
...
args *
Entity::attrFind(string name, double value) {
	stringstream _value;
	_value << value;
...
void
Entities::entityAdd(Entity ent) {
	entities.push_back(ent);
}

Entity *
Entities::entityAt(int pos) {
	if (entities.empty()) {
		return NULL;
	}
...
Entity *
Entities::entityFind(string name) {
	list<Entity>::iterator iter;
...
list <Entity *>
Entities::entitiesFilter(string name, string value) {
	list<Entity>::iterator iter;
	list<Entity *> elist;
...
list <Entity *>
Entities::entitiesFilter(string name, double value) {
	list<Entity>::iterator iter;
	list<Entity *> elist;
...
list <Entity *>
Entities::entitiesFilter(string name) {
	list<Entity>::iterator iter;
	list<Entity *> elist;
...
void
Entities::entitiesAttrAdd(string name, string value) {
	list<Entity>::iterator iter;
...
void
Entities::entitiesAttrAdd(string name, double value) {
	list<Entity>::iterator iter;
...
void
Entities::entitiesAttrMod(string name, string value) {
	entitiesAttrAdd(name, value);
}

void
Entities::entitiesAttrMod(string name, double value) {
	entitiesAttrAdd(name, value);
}

void
Entities::entitiesAttrDel(string name) {
	list<Entity>::iterator iter;
...
void
Entities::entityDelAll(void) {
	entities.clear();
}

int
Entities::entityNbr(void) {
	return entities.size();
}

string
Entities::entitiesDump(void) {
	static int e_id = 1;
	stringstream ret;
	list<Entity>::iterator iter;
